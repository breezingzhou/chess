...
class TreeNode():
  ...
  def expand(self, action_priors: list[tuple[Move, float]]):    # 这里把不合法的动作概率全部设置为0
    """通过创建新子节点来展开树"""
    for action, prob in action_priors:
      if action not in self._children:
        self._children[action] = TreeNode(self, prob)

  def select(self, c_puct: int) -> tuple[Move, "TreeNode"]:
    """
  ...
  def get_value(self, c_puct: int) -> float:
    """
...
class MCTS():
  ...
  def _playout(self, state: Board):
    ...
    if not end:
      ...
      if winner == None:    # 平局
        ...
        leaf_value = 1.0 if winner == state.current_turn else -1.0

    # 在本次遍历中更新节点的值和访问次数
    # 必须添加符号，因为两个玩家共用一个搜索树
    node.update_recursive(-leaf_value)

  def get_move_probs(self, state: Board, temperature: float = 1e-3) -> tuple[list[Move], np.ndarray]:
    """
    按顺序运行所有搜索并返回可用的动作及其相应的概率
    state:当前游戏的状态
    temperature:介于（0， 1]之间的温度参数
    """
  ...
  def update_with_move(self, last_move: Move):
    """
    在当前的树上向前一步，保持我们已经直到的关于子树的一切
    """
    assert last_move in self._current._children
    self._current = self._current._children[last_move]
...
class MCTSPlayer():
  ...
  def get_move(self, board: Board, temperature=1e-3):
    # 像alphaGo_Zero论文一样使用MCTS算法返回的pi向量
    move_probs = np.zeros(2086)

    moves, probs = self.mcts.get_move_probs(board, temperature)
    ...
    if self.is_selfplay:
      ...
      # 使用默认的temperature=1e-3，它几乎相当于选择具有最高概率的移动
